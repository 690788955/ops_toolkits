...
`cjg` just ran `trellis init` on a fresh clone, saw "Developer
...
- `.trellis/workspace/cjg/journal-*.md` — their session log,
...
- Check if `.trellis/workspace/cjg/` already exists — if yes, it's
  their journal from another machine and worth mentioning.
- Run `python ./.trellis/scripts/task.py list --assignee cjg` to
...
python ./.trellis/scripts/task.py archive 00-join-cjg
```
