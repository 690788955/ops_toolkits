...
# Docker 容器内通过 bash -c 执行脚本命令
# 用法: docker-exec-bash.sh <容器名或ID> <脚本内容>
# 示例: docker-exec-bash.sh ansible "cd /app && ./deploy.sh"
...
if [[ -z "$CONTAINER" ]]; then
  ...
  echo "用法: $0 <容器名或ID> <脚本内容>" >&2
  exit 1
fi

if [[ -z "$SCRIPT" ]]; then
  echo "错误: 未指定要执行的脚本内容" >&2
  echo "用法: $0 <容器名或ID> <脚本内容>" >&2
  exit 1
fi
